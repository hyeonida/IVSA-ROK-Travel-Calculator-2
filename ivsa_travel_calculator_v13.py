...
st.write("출발지/도착지 및 이동수단(버스, 기차, 비행기)을 선택하면 규정에 맞춰 가산금 및 초과 감액이 자동으로 통합 산출됩니다!")
...
with st.expander("📌 IVSA 교통비 환급 핵심 규정 요약 (항공/기차 포함)"):
    ...
    * **항공편(비행기) 추가금:**
        * 비행기 이용 시 소요 시간과 관계없이 **편도당 +15,000원**의 추가금이 부여됩니다.
    * **5만원 초과 감액:**
...
if trip_pattern in ["왕복", "편도"]:
    ...
    if trans_mode == "비행기 (항공편)":
        st.info("✈️ **항공편 추가금 적용:** 비행기 결제 금액에 편도당 15,000원이 추가됩니다")
        ...
        dur_options = [
            "2시간 30분 미만 (추가금 없음)",
            "2시간 30분 이상 - 3시간 30분 미만 (편도 +10,000원)",
            "3시간 30분 이상 (편도 +15,000원)"
        ]
