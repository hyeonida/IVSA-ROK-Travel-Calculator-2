...
# 페이지 설정
st.set_page_config(
    page_title="IVSA 임원진 교통비 환급 계산기 (통합 노선 매핑형)",
    page_icon="💳",
    layout="centered",
...
elif os.path.exists("ivsa_travel_map.png"):
    st.image("ivsa_travel_map.png", caption="IVSA 전국 수의과대학 공식 노선도 및 참고 요금 기준", use_container_width=True)
...
if is_jeju_trip:
    ...
    * **규정 적용 전 기준 합계 ($X$):** **{total_x:,}원**
    ...
    st.info("💡 계산된 환급 금액은 규정 기준을 적용한 금액이며, 최종 지급을 위해서는 영수증을 제출하셔야 합니다.")
